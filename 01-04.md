...
- [[**MITRE ATT&CK Enterprise FrameWork**](https://attack.mitre.org/)](#mitre-attck-enterprise-frameworkhttpsattackmitreorg)
    - [Perface](#perface)
    - [Initial Access (初始访问)](#initial-access-初始访问)
        - [Drive-by Compromise (路过式威胁)](#drive-by-compromise-路过式威胁)
            - [防御方式](#防御方式)
            - [检测手段](#检测手段)
        - [Exploit Public-Facing Application (利用大众化应用程序)](#exploit-public-facing-application-利用大众化应用程序)
            - [防御方式](#防御方式-1)
            - [检测手段](#检测手段-1)
        - [External Remote Services (外部远程服务)](#external-remote-services-外部远程服务)
            - [防御方式](#防御方式-2)
            - [检测手段](#检测手段-2)
        - [Hardware Additions (物理渗透硬件)](#hardware-additions-物理渗透硬件)
            - [防御方式](#防御方式-3)
            - [检测手段](#检测手段-3)
        - [Replication Through Removable Media (通过可移动媒介复制)](#replication-through-removable-media-通过可移动媒介复制)
            - [防御方式](#防御方式-4)
            - [检测手段](#检测手段-4)
        - [Spearphishing Attachment (鱼叉式钓鱼附件)](#spearphishing-attachment-鱼叉式钓鱼附件)
            - [防御方式](#防御方式-5)
            - [检测手段](#检测手段-5)
        - [Spearphishing Link (鱼叉式钓鱼链接)](#spearphishing-link-鱼叉式钓鱼链接)
            - [防御方式](#防御方式-6)
            - [检测手段](#检测手段-6)
        - [Spearphishing via Service (服务式鱼叉钓鱼)](#spearphishing-via-service-服务式鱼叉钓鱼)
            - [防御方式](#防御方式-7)
            - [检测手段](#检测手段-7)
        - [Supply Chain Compromise (供应链攻击)](#supply-chain-compromise-供应链攻击)
            - [防御方式](#防御方式-8)
            - [检测手段](#检测手段-8)
        - [Trusted Relationship (受信关系)](#trusted-relationship-受信关系)
            - [防御方式](#防御方式-9)
            - [检测手段](#检测手段-9)
        - [Valid Accounts (合法账号)](#valid-accounts-合法账号)
            - [防御方式](#防御方式-10)
            - [检测手段](#检测手段-10)
    - [Execution (执行)](#execution-执行)
        - [AppleScript (AppleScript)](#applescript-applescript)
            - [防御方式](#防御方式-11)
            - [检测手段](#检测手段-11)
        - [CMSTP (CMSTP-Microsoft连接管理器配置文件安装程序)](#cmstp-cmstp-microsoft连接管理器配置文件安装程序)
            - [防御方式](#防御方式-12)
            - [检测手段](#检测手段-12)
        - [Command-Line Interface (命令行界面)](#command-line-interface-命令行界面)
            - [防御方式](#防御方式-13)
            - [检测手段](#检测手段-13)
        - [Compiled HTML File (.chm文件)](#compiled-html-file-chm文件)
            - [防御方式](#防御方式-14)
            - [检测手段](#检测手段-14)
        - [Component Object Model and Distributed COM (COM & DCOM)](#component-object-model-and-distributed-com-com--dcom)
            - [防御方式](#防御方式-15)
            - [检测手段](#检测手段-15)
        - [Control Panel Items (控制面板项)](#control-panel-items-控制面板项)
            - [防御方式](#防御方式-16)
            - [检测手段](#检测手段-16)
        - [Dynamic Data Exchange (动态数据交换协议)](#dynamic-data-exchange-动态数据交换协议)
            - [防御方式](#防御方式-17)
            - [检测手段](#检测手段-17)
        - [Execution through API (通过API执行)](#execution-through-api-通过api执行)
            - [防御方式](#防御方式-18)
            - [检测手段](#检测手段-18)
        - [Execution through Module Load (通过模块加载执行)](#execution-through-module-load-通过模块加载执行)
            - [防御方式](#防御方式-19)
            - [检测手段](#检测手段-19)
        - [Execution for Client Execution (客户端执行的利用)](#execution-for-client-execution-客户端执行的利用)
            - [基于浏览器的利用](#基于浏览器的利用)
            - [办公应用](#办公应用)
            - [常见第三方应用](#常见第三方应用)
            - [防御方式](#防御方式-20)
        ...
        - [Launchctl (macOS)](#launchctl-macos)
            - [防御方式](#防御方式-23)
            - [检测手段](#检测手段-23)
        - [Local Job Scheduling (本地作业调度)](#local-job-scheduling-本地作业调度)
            - [cron](#cron)
            - [at](#at)
            - [launchd](#launchd)
            - [防御方式](#防御方式-24)
            - [检测手段](#检测手段-24)
        - [LSASS Driver (LSASS驱动程序)](#lsass-driver-lsass驱动程序)
            - [防御方式](#防御方式-25)
            - [检测手段](#检测手段-25)
        ...
        - [Rundll32](#rundll32)
        - [Scheduled Task (计划任务)](#scheduled-task-计划任务)
        - [Scripting](#scripting)
        ...
        - [User Execution (用户执行)](#user-execution-用户执行)
    ...
    - [Persistence (持久化)](#persistence-持久化)
        - [.bash_profile and .bashrc](#bash_profile-and-bashrc)
        ...
        - [BITS Jobs (BITS-Windows后台智能传输服务-利用)](#bits-jobs-bits-windows后台智能传输服务-利用)
            - [背景](#背景)
            - [武器化](#武器化)
            - [防御方式](#防御方式-29)
            - [检测](#检测)
        ...
        - [Component Firmware (组件固件)](#component-firmware-组件固件)
            - [背景](#背景-1)
            - [武器化](#武器化-1)
            - [防御方式](#防御方式-30)
            - [检测](#检测-1)
        - [Component Object Model Hijacking (COM劫持)](#component-object-model-hijacking-com劫持)
            - [背景](#背景-2)
            - [利用场景](#利用场景)
            - [防御方式](#防御方式-31)
        - [检测手段](#检测手段-29)
        - [Create Account](#create-account)
        - [DLL Search Order Hijacking (DLL搜索顺序劫持)](#dll-search-order-hijacking-dll搜索顺序劫持)
    ...
    - [Privilege Escalation (提权)](#privilege-escalation-提权)
        - [Access Token Manipulation (操作访问令牌)](#access-token-manipulation-操作访问令牌)
        ...
        - [Bypass User Account Control (UAC绕过)](#bypass-user-account-control-uac绕过)
        ...
        - [Extra Window Memory Injection (窗口内存注入)](#extra-window-memory-injection-窗口内存注入)
        ...
        - [Process Injection (进程注入)](#process-injection-进程注入)
    ...
    - [Defense Evasion (防御规避)](#defense-evasion-防御规避)
        - [Access Token Manipulation](#access-token-manipulation)
        - [Application Access Token (应用程序访问令牌) (SaaS&Office 365)](#application-access-token-应用程序访问令牌-saasoffice-365)
            - [背景](#背景-3)
            - [利用场景](#利用场景-1)
            - [防御方式](#防御方式-32)
            - [检测](#检测-2)
        - [Binary Padding (二进制填充) (all)](#binary-padding-二进制填充-all)
            - [背景](#背景-4)
            - [利用场景](#利用场景-2)
            - [防御方式](#防御方式-33)
            - [检测](#检测-3)
        - [BITS Jobs](#bits-jobs)
        - [Bypass User Account Control (UAC-用户账户控制-绕过) (Windows)](#bypass-user-account-control-uac-用户账户控制-绕过-windows)
        - [Clear Command History (清除命令历史记录) (Linux&MacOS)](#clear-command-history-清除命令历史记录-linuxmacos)
            - [背景](#背景-5)
            - [利用场景](#利用场景-3)
            - [防御方式](#防御方式-34)
            - [检测](#检测-4)
        - [CMSTP](#cmstp)
        - [Code Signing (代码签名) (MacOS&Windows)](#code-signing-代码签名-macoswindows)
            - [背景](#背景-6)
            - [利用场景](#利用场景-4)
            - [防御方式](#防御方式-35)
            - [检测](#检测-5)
        - [Compile After Delivery (交付后编译) (All)](#compile-after-delivery-交付后编译-all)
            - [背景](#背景-7)
            - [利用场景](#利用场景-5)
            - [防御方式](#防御方式-36)
            - [检测](#检测-6)
        ...
        - [Connection Proxy (连接代理) (All)](#connection-proxy-连接代理-all)
            - [背景](#背景-8)
            - [利用场景](#利用场景-6)
            - [防御方式](#防御方式-37)
            - [检测](#检测-7)
        ...
        - [DCShadow (DCShadow)](#dcshadow-dcshadow)
            ...
            - [防御方式](#防御方式-38)
            - [检测](#检测-8)
        - [Deobfuscate/Decode Files or Information (反混淆/解码)](#deobfuscatedecode-files-or-information-反混淆解码)
            - [背景](#背景-10)
            - [利用场景](#利用场景-8)
            - [防御方式](#防御方式-39)
            - [检测](#检测-9)
        - [Disabling Security Tools (瘫痪安全服务) (All)](#disabling-security-tools-瘫痪安全服务-all)
            - [背景](#背景-11)
            - [利用场景](#利用场景-9)
            - [防御方式](#防御方式-40)
            - [检测](#检测-10)
        - [DLL Search Order Hijacking](#dll-search-order-hijacking-1)
        - [DLL Side-Loading (DLL旁路加载) (Windows)](#dll-side-loading-dll旁路加载-windows)
            - [背景](#背景-12)
            - [利用场景](#利用场景-10)
            - [防御方式](#防御方式-41)
            - [检测](#检测-11)
        - [Execution Guardrails (执行边界) (All)](#execution-guardrails-执行边界-all)
            - [背景](#背景-13)
            - [利用场景](#利用场景-11)
            - [防御方式](#防御方式-42)
            - [检测](#检测-12)
            - [补充](#补充)
        - [Exploitation for Defense Evasion (漏洞利用免杀) (All)](#exploitation-for-defense-evasion-漏洞利用免杀-all)
            - [背景](#背景-14)
            - [利用场景](#利用场景-12)
            - [防御方式](#防御方式-43)
            - [检测](#检测-13)
        ...
        - [File and Directory Permissions Modification (文件目录权限修改) (All)](#file-and-directory-permissions-modification-文件目录权限修改-all)
            - [背景](#背景-15)
            - [利用场景](#利用场景-13)
            - [防御方式](#防御方式-44)
            - [检测](#检测-14)
            - [补充](#补充-2)
        - [File Deletion (文件删除) (All)](#file-deletion-文件删除-all)
            - [背景](#背景-16)
            - [利用场景](#利用场景-14)
            - [防御方式](#防御方式-45)
            - [检测](#检测-15)
        ...
        - [LC_MAIN Hijacking](#lc_main-hijacking)
    ...
    - [Credential Access (凭证访问)](#credential-access-凭证访问)
    ...
    - [Discovery (嗅探扫描)](#discovery-嗅探扫描)
        ...
        - [Query Registry (查询注册表)](#query-registry-查询注册表)
    ...
    - [Lateral Movement (横向移动)](#lateral-movement-横向移动)
        ...
        - [Remote Services (远程服务)](#remote-services-远程服务)
