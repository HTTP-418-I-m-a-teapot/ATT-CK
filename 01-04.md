...
- [MITRE ATT&amp;CK Enterprise FrameWork](#mitre-attampck-enterprise-framework)
  ...
  - [Persistence (持久化)](#persistence-%e6%8c%81%e4%b9%85%e5%8c%96)
    - [.bash_profile and .bashrc (.bash_profile &amp; .bashrc)](#bashprofile-and-bashrc-bashprofile-amp-bashrc)
...
**特权账户管理**|限制对root帐户的访问，并通过适当的特权分离和限制特权升级机会来防止用户加载内核模块和扩展
...
plist文件的权限必须是 root：wheel，但是它指向的脚本或程序没有这样的要求。因此，不良配置有可能使对手修改当前的启动守护程序的可执行文件并获得持久性或特权升级。

#### 防御方式

缓解|描述
:--:|:--
**用户帐号管理**|限制用户帐户的特权并修复特权升级向量，以便只有授权的管理员才能创建新的启动守护程序
